...
export default function Page({ params }: { params: { locale: string } }) {
  const { data } = useQuery(queries.cmsPostList, {
    variables: {
      categoryIds: ["tqwE3QglvRHNtC9YTQRnS"],
    },
  });

  const posts = data?.cpPostList?.posts?.filter((post: any) =>
    post?.categoryIds.includes("tqwE3QglvRHNtC9YTQRnS"),
  );

  return (
    <PageShell title="Жүдо">
      ...
      {posts && posts.length > 0 && (
        <ul className="cat-list" style={{ marginTop: "2rem" }}>
          {posts.map((post: any) => (
            <li key={post._id}>
              <a href={`/${params.locale}/judo/${post._id}`} className="noajax">
                {post.title}
              </a>
            </li>
          ))}
