...
export default function Page({ params }: { params: { locale: string } }) {
  const { data } = useQuery(queries.cmsPostList, {
    variables: {
      categoryIds: [CATEGORY.CHEERLEADING],
    },
  });

  const posts = data?.cpPostList?.posts?.filter((post: any) =>
    post?.categoryIds.includes(CATEGORY.CHEERLEADING),
  );

  return (
    <PageShell title="Чирлидинг">
      ...
      {posts && posts.length > 0 && (
        <ul className="cat-list" style={{ marginTop: "2rem" }}>
          {posts.map((post: any) => (
            <li key={post._id}>
              <a href={`/${params.locale}/cheerleading/${post._id}`} className="noajax">
                {post.title}
              </a>
            </li>
          ))}
