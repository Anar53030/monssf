...
export default function Page({ params }: { params: { locale: string } }) {
  const { data } = useQuery(queries.cmsPostList, {
    variables: {
      categoryIds: ["JYRDJEe2IwHo8oZcmv9ub"],
    },
  });

  const posts = data?.cpPostList?.posts?.filter((post: any) =>
    post?.categoryIds.includes("JYRDJEe2IwHo8oZcmv9ub"),
  );

  return (
    <PageShell title="Шатар">
      ...
      {posts && posts.length > 0 && (
        <ul className="cat-list" style={{ marginTop: "2rem" }}>
          {posts.map((post: any) => (
            <li key={post._id}>
              <a href={`/${params.locale}/chess/${post._id}`} className="noajax">
                {post.title}
              </a>
            </li>
          ))}
