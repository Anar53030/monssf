...
export default function Page({ params }: { params: { locale: string } }) {
  const { data } = useQuery(queries.cmsPostList, {
    variables: {
      categoryIds: ["aNXkN71LhYLafSDiOkP8u"],
    },
  });

  const posts = data?.cpPostList?.posts?.filter((post: any) =>
    post?.categoryIds.includes("aNXkN71LhYLafSDiOkP8u"),
  );

  return (
    <PageShell title="Волейбол">
      ...
      {posts && posts.length > 0 && (
        <ul className="cat-list" style={{ marginTop: "2rem" }}>
          {posts.map((post: any) => (
            <li key={post._id}>
              <a href={`/${params.locale}/volleyball/${post._id}`} className="noajax">
                {post.title}
              </a>
            </li>
          ))}
