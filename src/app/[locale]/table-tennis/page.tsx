...
export default function Page({ params }: { params: { locale: string } }) {
  const { data } = useQuery(queries.cmsPostList, {
    variables: {
      categoryIds: ["rTrNmuH592ZCKEg_Q_5s3"],
    },
  });

  const posts = data?.cpPostList?.posts?.filter((post: any) =>
    post?.categoryIds.includes("rTrNmuH592ZCKEg_Q_5s3"),
  );

  return (
    <PageShell title="Ширээний теннис">
      ...
      {posts && posts.length > 0 && (
        <ul className="cat-list" style={{ marginTop: "2rem" }}>
          {posts.map((post: any) => (
            <li key={post._id}>
              <a href={`/${params.locale}/table-tennis/${post._id}`} className="noajax">
                {post.title}
              </a>
            </li>
          ))}
